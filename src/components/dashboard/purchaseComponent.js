...
function PurchaseComponent() {
    ...
    const fetchAll = async () => {
        ...
        if(gotId === 0){
            ...
            await Instance.get(`fetchPurchId`)
            .then(res => {
                id = res.data.value;
                setId(res.data.value);
                setInvoice('MEDC-S0'+id);
            })
            .catch( error => {
                console.log(error);
            })
            ...
            await Instance.get(`fetchPurchIdSub/${gotId}`)
            .then(res => {
                setInvoice(res.data.data.inv_id);
                setCustomer(res.data.data.supplier_id);
            })
            .catch( error => {
                console.log(error);
            })
    ...
    const deleteProduct = async (id) => {
        ...
        await Instance.post(`deleteData/purchase_product`,data , { headers }).
            then((res) => {
    ...
    const saveInvoice = async (e) => {
        ...
        if(gotId === 0){
            if(customer !== '' ){
                ...
                await Instance.post(`insert/purchase`,inv_data , { headers }).
                    then((res) => {
                ...
                message.error('Select supplier Name');
            }
        }else{
            if(customer !== '' ){
                let inv_data = {
                ...
                await Instance.post(`updateData/purchase/${gotId}`,inv_data , { headers }).
                    then((res) => {
        
                        if(res.status === 200 || res.status === '200'){
                            message.success(res.data.msg);
                            history.push("/dashboard/listpurchase") 
                        }
                        else{
                            message.error('somthing went wrong');
                        }
        ...
        <>
            <div className="row mt-4">
                <div className="form-group col-md-6">
                    ...
                    <input className="form-control" type="text" value={ invoice } readOnly></input>
                </div>
                <div className="form-group col-md-6">
            ...
            { show ? 
            <PurchaseProductModal close={ handleClose } grandTotal ={ grandTotal } s_id= { id } />
            : null }
